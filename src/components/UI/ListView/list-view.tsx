...
import './index.scss'
import { TinyIcon } from '../IconSets'

const { useState, useRef, useEffect, useImperativeHandle } = React

const prefixCls = 'ai--list-view'
        ...
        {pullDownRefresh && (
          <div className={`${prefixCls}-pull-down-tip`}>
            {pullDownStatus !== 0 ? (
              <div>
                {pullDownStatus === 1 &&
                  (refreshTips?.loading ? (
                    ...
                    <TinyIcon type="iconspinner" spin size={20} />
                  ))}
                {pullDownStatus === 2 &&
                  (refreshTips?.success ? (
                    refreshTips?.success.icon
                  ) : (
                    <TinyIcon type="icondanxuan" size={20} />
                  ))}
                {pullDownStatus === 3 &&
                  (refreshTips?.error ? (
              ...
              <div>
                <TinyIcon type="iconshifangshuaxin" size={20} />
                <span>
                  {translateY > 60
            ...
            {noMore ? (
              ...
              <div>
                {upLoadTips?.error ? (
                  ...
                  <>
                    <TinyIcon type={'times-circle'} size={18} />
                    <span>加载失败</span>
                  </>
                )}
              </div>
